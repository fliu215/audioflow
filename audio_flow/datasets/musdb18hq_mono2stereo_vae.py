...
class MUSDB18HqMono2StereoVAE(Dataset):
    ...
    def load_meta(self):
        ...
        mono_paths = sorted(list(Path(self.root, self.split).rglob('*mono_vae.h5')))
        mono_paths = [str(s) for s in mono_paths]
        stereo_paths = [s.replace("mono_vae.h5", "stereo_vae.h5") for s in mono_paths]
