...
from audio_flow.models.aligner_ljspeech_02 import Aligner_ljspeech_02
from audio_flow.utils import get_single_value


class Adapter_ljspeech_02(nn.Module): 
    def __init__(self, dim: int, max_length: int, **kwargs):
        super().__init__()

        # Text encoder
        self.t5_encoder = T5()
        self.t5_aligner = Aligner_ljspeech_02(self.t5_encoder.dim, dim, max_length)

        # TTS encoder
        self.char_encoder = CharEncoder()
        self.char_embedder = nn.Embedding(self.char_encoder.vocab_size, dim)
        self.char_aligner = Aligner_ljspeech_02(dim, dim, max_length)
