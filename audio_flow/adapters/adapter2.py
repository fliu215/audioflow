...
from audio_flow.models.aligner2 import Aligner2
from audio_flow.utils import get_single_value, build_attention_mask
...
class Adapter2(nn.Module): 
    def __init__(self, dim: int, max_length: int, **kwargs):
        ...
        self.char_aligner = Aligner2(dim, dim, max_length)
    ...
    def forward(self, data: dict) -> Tensor:
        ...
        if task == "text_to_music":
            ...
            return self.get_tta_condition(data)
    ...
    def get_tts_condition(self, data: dict) -> Tensor:
        ...
        length = max(data["latent_length"])

        task = self.t5_encoder(data["task"])
        task = task.mean(dim=1, keepdims=True).repeat(1, length, 1)  # (b, l, d)
        device = task.device

        content, content_padding_mask = self.char_encoder(data["content"])  # (b, l_content, d)
        content = self.char_embedder(content)  # (b, l_content, d)

        a1 = torch.cat([content_padding_mask, data["target_audio_mask"].to(device)], dim=1)
        attn_mask = build_attention_mask(a1)  # (b, 1, l1+l2, l1+l2)
        content = self.char_aligner(content, attn_mask)  # (b, l2, d)
